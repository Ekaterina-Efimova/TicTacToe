 ...
 * Содержит методы для обработки ходов, проверки состояния игры и управления игроками.
 */
public class TicTacToeServiceImpl implements TicTacToeService {
  ...
  private void minimax(Game game, Game oldGame) {
    ...
    if (!Validate.validateMove(computerMove, RoleEnum.PLAYER2, game.getBoard().getBoard()[computerMove[0]][computerMove[1]]))
      throw new IllegalArgumentException("Некорректный ход компьютера");
    else
      game.getBoard().setElement(computerMove[0], computerMove[1], oldGame.getPlayer1().equals(UUID.fromString("00000000-0000-0000-0000-000000000000")) ? 1 : 2);
    changeTurn(game);
  }
  /**
  ...
  /**
   * Обрабатывает ход игры, переключая игрока и выполняя ход компьютера, если необходимо.
   *
   * @param game текущее состояние игры
   * @param oldGame предыдущее состояние игры
   */
